// CircularGallery（曲面旋转画廊）— 原生 ESM 版，改编自 React Bits 组件
// 事件仅绑定到容器、加入自动旋转与 ResizeObserver，适配静态页面与面板显隐
...
class App {
  constructor(container, { items, bend = 3, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05, autoSpeed = 0.008 } = {}) {
    ...
    if (window.ResizeObserver) { this.ro = new ResizeObserver(() => this.onResize()); this.ro.observe(this.container); }
  }
  createRenderer() {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const gi = items && items.length ? items : [];
  ...
  update() {
    ...
    if (!this.isDown) this.scroll.target += this.autoSpeed; // 自动持续旋转
...
function initTerrainGallery() {
  const el = document.getElementById('terrain-gallery');
  if (!el) return;
  const items = [1, 2, 3, 4, 5, 6].map(n => ({ image: `assets/terrain/terrain-${n}.jpg`, text: `情绪地形 0${n}` }));
  try {
    new App(el, { items, bend: 3, textColor: '#8C8A84', borderRadius: 0.06, font: "600 22px 'Plus Jakarta Sans', sans-serif", scrollSpeed: 2, scrollEase: 0.05, autoSpeed: 0.008 });
    const fb = document.getElementById('terrain-gallery-fallback');
    if (fb) fb.style.display = 'none';
  } catch (e) {
    console.warn('CircularGallery init failed, keeping static fallback:', e);
  }
}

if (document.readyState !== 'loading') initTerrainGallery();
else document.addEventListener('DOMContentLoaded', initTerrainGallery);
